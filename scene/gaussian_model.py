...
class GaussianModel:
    ...
    def create_from_pcd(
        ...
        if skybox_points > 0:
            ...
            print("sky x range", torch.min(skybox_xyz[:, 0]), torch.max(skybox_xyz[:, 0]))
            print("sky y range", torch.min(skybox_xyz[:, 1]), torch.max(skybox_xyz[:, 1]))
            print("sky z range", torch.min(skybox_xyz[:, 2]), torch.max(skybox_xyz[:, 2]))
            skybox_xyz += mean.cpu()
