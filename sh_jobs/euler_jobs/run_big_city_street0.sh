...
python json_to_colmap.py --base_dir=/cluster/work/cvl/qimaqi/cvpr_2025/datasets/MatrixCity/colmap_street/camera_calibration/rectified/ --view_name=street --pc_path=/cluster/work/cvl/qimaqi/cvpr_2025/datasets/MatrixCity/small_city_pc/point_cloud_ds20/street/Block_all_.ply

# colmap model_converter --input_path /cluster/work/cvl/qimaqi/cvpr_2025/datasets/MatrixCity/colmap_street/camera_calibration/aligned/sparse/mc --output_path=/cluster/work/cvl/qimaqi/cvpr_2025/datasets/MatrixCity/colmap_street/camera_calibration/aligned/sparse/mc --output_type=BIN

# python preprocess/generate_chunks_mc.py --mc_dir /cluster/work/cvl/qimaqi/cvpr_2025/datasets/MatrixCity/colmap_street/  --min_n_cams 20 --chunk_size 200
...
# CHUNK_NAMES=("0_2")
#("0_2" "0_3" "0_4" "0_5" "0_6" "1_1" "1_2" "1_3" "1_4" "1_5" "1_6" "1_7" "1_8")
# 
...
# CHUNK_NAME=0_2 
# python train_single.py -s ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME} --model_path ${DATASET_DIR}/output/  -d ${DATASET_DIR}/camera_calibration/rectified/depths --exposure_lr_init 0.0 --eval --skip_scale_big_gauss
...
# python render_hierarchy.py -s ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME} --model_path ${DATASET_DIR}/output/ --hierarchy ${DATASET_DIR}/output/trained_chunks/${CHUNK_NAME}/hierarchy.hier_opt --out_dir ${DATASET_DIR}/output/render_0_2 --eval



# for CHUNK_NAME in "${CHUNK_NAMES[@]}"; do 
#     python -u train_single.py -s ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME} --model_path ${DATASET_DIR}/output/trained_chunks/${CHUNK_NAME} -i ${DATASET_DIR}/camera_calibration/rectified/images  --scaffold_file ${DATASET_DIR}/output/scaffold/point_cloud/iteration_30000 --skybox_locked --bounds_file ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME}  --port 6010 -d ${DATASET_DIR}/camera_calibration/rectified/depths
...
#     # post 
#     python -u train_post.py -s ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME} --model_path ${DATASET_DIR}/output/trained_chunks/${CHUNK_NAME} --hierarchy ${DATASET_DIR}/output/trained_chunks/${CHUNK_NAME}/hierarchy.hier --iterations 15000 --feature_lr 0.0005 --opacity_lr 0.01 --scaling_lr 0.001  -i ${DATASET_DIR}/camera_calibration/rectified/images --scaffold_file ${DATASET_DIR}/output/scaffold/point_cloud/iteration_30000 --skybox_locked --bounds_file ${DATASET_DIR}/camera_calibration/chunks/${CHUNK_NAME} --port 6010 -d ${DATASET_DIR}/camera_calibration/rectified/depths    
# done
